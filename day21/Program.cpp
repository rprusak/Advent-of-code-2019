...
Program::Program(const std::string& inputProgram, ProgramInputInterface& input)
    : _output{0},
      _programPointer{0},
      _relativeBase{0},
      _isHalted{false},
  ...
  while (_programPointer < _program.size()) {
    ...
    switch (instruction.operationCode) {
      ...
      case OperationCode::INPUT:
        handleINPUT(instruction, _input.getValue());
        break;
      case OperationCode::OUTPUT:
        handleOUTPUT(instruction);
        std::cout << (char)_output;
        break;
...
long Program::getOutput() const { return _output; }
