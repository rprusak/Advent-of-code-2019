...
bool isNotDecreasing(unsigned long value) {
  ...
  for (size_t i = 0; i < str.size() - 1; i++) {
    if (str[i] - 48 > str[i + 1] - 48) return false;
  }
...
bool hasOnlySameTwoValuesInRow(unsigned long value) {
  ...
  while (currentIndex < str.size()) {
    if (str[currentIndex] == str[currentIndex + 1]) {
      ...
      if (currentCounter == 2) doubleFound = true;
      currentCounter = 1;
    }
    currentIndex++;
  }

  if (currentCounter == 2) doubleFound = 2;
...
int main() {
  ...
  for (unsigned long i = start; i <= end; i++) {
    if (hasSameTwoValuesInRow(i) && isNotDecreasing(i) &&
        hasOnlySameTwoValuesInRow(i))
      counter++;
  }
