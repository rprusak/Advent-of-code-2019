...
int main(int argc, char** argv) {
    ...
    auto hasZeroVelocity = [](const Point& p) {
        return p.hasZeroVelocity();
    };

    auto isInInitialState = [](const Point& p) {
        return p.isInIntialState();
    };
    ...
    while (true) {
        ...
        bool zeroVelocity = std::all_of(points.begin(), points.end(), hasZeroVelocity);
        if (zeroVelocity)
            std::cout << "zero v" << std::endl;

        if (zeroVelocity && std::all_of(points.begin(), points.end(), isInInitialState))
            break;
    }
