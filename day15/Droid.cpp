...
Droid::Droid(ProgramInterface& program)
    : _program(program), _dev{}, _rng{_dev()} {
...
void Droid::run() {
  ...
  //    std::cout << "unvisited count " << _visitedStatus.unvisitedCount() <<
  //    std::endl; print();
...
bool Droid::hasNotDiscoveredFieldsAround() const {
  ...
  return std::find_if(directions.begin(), directions.end(), isNotDiscovered) !=
         directions.end();
}
...
std::vector<Droid::Direction> Droid::getNotDiscoveredFieldsAround() const {
  ...
  std::copy_if(directions.begin(), directions.end(), std::back_inserter(result),
               isNotDiscovered);
  return result;
}
...
Droid::Direction Droid::getMoveDirection() {
  ...
  std::copy_if(possibleDirections.begin(), possibleDirections.end(),
               std::back_inserter(emptyFields), isEmptyField);

  std::uniform_int_distribution<std::mt19937::result_type> dist(
      0, emptyFields.size() - 1);
...
long Droid::getShortestPathLength() {
  ...
  for (const auto& [point, field] : _area) {
    if (field != FieldType::WALL) points.push_back(point);
  }

  std::vector<std::vector<int>> graph{points.size()};
  for (auto& record : graph) record.resize(points.size(), 0);

  for (const auto& point : points) {
    _currentPosition = point;
    auto pointIndex = std::distance(
        points.begin(), std::find(points.begin(), points.end(), point));
    auto movableFields = getMovableFieldsAround();
    for (const auto& [secondPoint, field] : movableFields) {
      auto secondPointIndex = std::distance(
          points.begin(), std::find(points.begin(), points.end(), secondPoint));
  ...
  auto startIndex = std::distance(
      points.begin(), std::find(points.begin(), points.end(), Point{0, 0}));
  auto destinationIndex = std::distance(
      points.begin(), std::find(points.begin(), points.end(), Point{-16, -12}));
...
std::vector<Droid::Direction> Droid::getMovableFieldsAround() const {
  ...
  auto isMovableField = [this](const Direction& direction) {
    return this->_area.count(direction.first) &&
           this->_area.at(direction.first) != FieldType::WALL;
  };

  std::vector<Direction> movableFields;
  std::copy_if(directions.begin(), directions.end(),
               std::back_inserter(movableFields), isMovableField);
...
std::vector<Droid::Point> Droid::getEmptyFieldsAround() const {
  ...
  auto isEmptyField = [this](const Direction& direction) {
    return this->_area.count(direction.first) &&
           this->_area.at(direction.first) == FieldType::EMPTY;
  };

  for (const auto& direction : directions) {
    if (isEmptyField(direction)) emptyPoints.push_back(direction.first);
  }
