...
FuelCalculator::FuelCalculator(const std::string &inputPath) : _inputPath(inputPath) {}

std::vector<FuelCalculator::Ingredient> FuelCalculator::parseIngredients(const std::string &str) {
...
std::vector<FuelCalculator::Recipe> FuelCalculator::parseInput(const std::vector<std::string> &input) {
...
unsigned long FuelCalculator::calculateFuelCost(const std::map<std::string, Recipe> &recipes, unsigned long fuelNeed) {
    ...
    auto isBasicIngredient = [&recipes](const std::list<FuelCalculator::Ingredient>::value_type & val) {
        return recipes.at(val.name).inputIngredients.size() == 1 && recipes.at(val.name).inputIngredients.front().name == "ORE";
    };
    ...
    while (!neededIngredients.all_of(isBasicIngredient)) {
        ...
        auto factor = getFactor(ingredientRecipe.outputIngredient.count, neededIngredient.count);

        auto leftoversCount = ingredientRecipe.outputIngredient.count * factor - neededIngredient.count;
        if (leftoversCount > 0) {
    ...
    for (const auto& [ingredientName, ingredientCount] : neededIngredients) {
        ...
        auto factor = getFactor(ingredientRecipe.outputIngredient.count, ingredientCount);
...
unsigned long FuelCalculator::getFactor(unsigned long recipeCount, unsigned long ingredientCount) {
...
unsigned long FuelCalculator::calculateOrePerFuel(unsigned long fuelNeed) {
    ...
    std::transform(recipes.begin(), recipes.end(), std::inserter(recipesMap, recipesMap.end()), recipeToPair);
