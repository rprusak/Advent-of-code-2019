...
private:
    ...
    public:
        bool empty() {
            if (std::list<FuelCalculator::Ingredient>::empty())
                return true;
    ...
    unsigned long calculateFuelCost(const std::map<std::string, Recipe> &recipes, unsigned long fuelNeed);
    static unsigned long getFactor(unsigned long recipeCount, unsigned long ingredientCount);
};
