...
std::string findRoot(const std::vector<Orbit>& orbits,
                     const std::set<std::string>& planets) {
  for (const auto& planet : planets) {
    auto planetIsOnOrbit = [planet](const Orbit& orbit) {
      return orbit.orbitPlanet == planet;
    };
    if (std::find_if(orbits.begin(), orbits.end(), planetIsOnOrbit) ==
        orbits.end()) {
      return planet;
    }
  }
...
void countOrbits(const std::string& root, const OrbitsMap& orbitsMap,
                 OrbitsCounters& orbitsCounters) {
  auto& rootCounter = orbitsCounters[root];
  if (!orbitsMap.count(root)) return;
...
bool orbitsPlanetsHaveSourcePlanet(const OrbitsMap& orbitsMap,
                                   const std::string& root,
                                   const std::string& source,
                                   std::vector<std::string>& path) {
  const auto& orbitPlanets = orbitsMap.at(root);

  if (root == "K") std::cout << "dd" << std::endl;
  ...
  for (const auto& orbitPlanet : orbitPlanets) {
    if (orbitPlanet == source) {
      ...
      continue;
    } else if (orbitsPlanetsHaveSourcePlanet(orbitsMap, orbitPlanet, source,
                                             path)) {
      path.push_back(root);
      return true;
    }
...
std::vector<std::string> findPath(const OrbitsMap& orbitsMap,
                                  const std::string& root,
                                  const std::string& source) {
  std::vector<std::string> path;
...
int main(int argc, char** argv) {
  ...
  std::transform(input.begin(), input.end(), inputOrbits.begin(),
                 stringToOrbit);
  ...
  for (const auto& planet : pathToYOU) {
    if (std::find(pathToSAN.begin(), pathToSAN.end(), planet) !=
        pathToSAN.end()) {
      searchPlanet = planet;
      break;
    }
  ...
  std::cout << std::distance(pathToSAN.begin(), it1) +
                   std::distance(pathToYOU.begin(), it2)
            << std::endl;
  return 0;
}
