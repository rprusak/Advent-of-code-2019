...
unsigned findPixel(const std::string& input, size_t widthIndex, size_t heightIndex) {
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < input.size(); i += width * height) {
        auto zerosCount = std::count(input.begin() + i, input.begin() + i + (width * height), '0');
        if (zerosCount < fewestZerosCount) {
            fewestZerosCount = zerosCount;
            auto oneCount = std::count(input.begin() + i, input.begin() + i + (width * height), '1');
            auto twoCount = std::count(input.begin() + i, input.begin() + i + (width * height), '2');
