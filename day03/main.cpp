...
public:
    ...
    std::optional<Point> getCrossingPoint(const Line& secondLine) const {
        if (isHorizontal() && secondLine.isHorizontal())
            return std::nullopt;

        if (isVertical() && secondLine.isVertical())
            return std::nullopt;
...
private:
    std::optional<Point> getCrossingPoint(const Line& horizontal, const Line& vertical) const {
        if (!horizontalMatch(horizontal, vertical))
            return std::nullopt;

        if (!verticalMatch(horizontal, vertical))
            return std::nullopt;
...
//    std::string secondWire{"U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"};
    ...
    << " " << std::abs(crossingPoints.front().x) + std::abs(crossingPoints.front().y) << std::endl;
