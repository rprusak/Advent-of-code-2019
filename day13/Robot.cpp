...
unsigned Robot::getBlockTilesCount() const {
    auto isBlockTile = [](const auto& p) {
        return p.second == TileId::BLOCK;
    };
...
long Robot::getScore() const {
    return _score;
}

long Robot::getInput() {
    auto isBall = [](const auto& p) {
        return p.second == TileId::BALL;
    };

    auto isPaddle = [](const auto& p) {
        return p.second == TileId::PADDLE;
    };
