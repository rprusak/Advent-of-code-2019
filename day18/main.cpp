...
class Vault : public std::map<Point, char> {
 public:
  Point getStartingPoint() {
    auto isStartingPoint = [](const auto& val) { return val.second == '@'; };
  ...
  bool isFullyVisited() const {
    auto isVisited = [](const auto& val) { return val.second == '.'; };
 ...
 private:
  std::vector<Point> getNearPoints(const Point& p) const {
    std::vector<Point> nearPoints{Point{p._x - 1, p._y}, Point{p._x + 1, p._y},
                                  Point{p._x, p._y - 1}, Point{p._x, p._y + 1}};

    auto pointNotExists = [this](const Point& p) { return count(p) == 0; };

    auto it =
        std::remove_if(nearPoints.begin(), nearPoints.end(), pointNotExists);
    nearPoints.erase(it, nearPoints.end());

    return nearPoints;
  }

  static bool isKey(char c) { return islower(c); }

  static bool isDoor(char c) { return isupper(c); }
};
...
int main() {
  std::string input1{
      "/home/rafal/Workspace/advent_of_code_2019/day18/input1.txt"};
