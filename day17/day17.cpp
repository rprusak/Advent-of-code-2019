...
class Line : public std::vector<Point> {
 public:
  std::optional<Point> getIntersectionPoint(const Line& line) const {
    ...
    if (it == line.end()) return std::nullopt;

    auto p = *it;

    if (p == front() || p == back()) return std::nullopt;
  ...
  for (size_t y = 0; y < inputHeight; y++) {
    ...
    for (size_t x = 0; x < inputWidth; x++) {
      ...
      if (c == '#') {
        ...
        if (line.size() > 1) horizontalLines.push_back(line);
        line.clear();
      }
    }

    if (!line.empty() && line.size() > 1) horizontalLines.push_back(line);
  }
  ...
  for (size_t x = 0; x < inputWidth; x++) {
    ...
    for (size_t y = 0; y < inputHeight; y++) {
      ...
      if (c == '#') {
        ...
        if (line.size() > 1) verticalLines.push_back(line);
        line.clear();
      }
    }

    if (!line.empty() && line.size() > 1) verticalLines.push_back(line);
  }

  return verticalLines;
}

unsigned long getSumOfTheAlignmentParameters(
  ...
  for (const auto& verticalLine : verticalLines) {
    for (const auto& horizontalLine : horizontalLines) {
      auto intersectionPoint =
          verticalLine.getIntersectionPoint(horizontalLine);
  ...
  unsigned long sum{0};
  for (const auto& p : intersectionPoints) sum += p._x * p._y;
