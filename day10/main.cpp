...
std::vector<Point> convertToPoints(const std::vector<std::string>& programInput) {
...
Line findLineBetweenPoints(const Point& sourcePoint, const Point& destinationPoint) {
    Line l;
...
unsigned findBestPoint(const std::vector<Point>& points) {
    ...
    for (auto sourcePointIt = points.begin(); sourcePointIt != points.end(); sourcePointIt++) {
        for (auto destinationPointIt = sourcePointIt + 1; destinationPointIt != points.end(); destinationPointIt++) {
